...
import { fetchGames, type GameListEntry, type Kind, type Scope } from "../lib/games";
...
function outcome(g: GameListEntry): { text: string; tone: Tone } {
  ...
  if (g.opponent_kind === "agent") {
    // AI vs AI match — label by which side won
    if (g.winner === "south") return { text: `${g.agent_id} won`, tone: "win" };
    if (g.winner === "north") return { text: `${g.opponent_agent_id ?? "north"} won`, tone: "loss" };
...
export function GamesPage({ scope, kind }: Props) {
  ...
  useEffect(() => {
    setGames(null);
    fetchGames(scope, kind, page).then((res) => {
      setGames(res.items);
      setTotal(res.total);
    });
  }, [scope, kind, page]);
  ...
  return (
    <div className="min-h-screen bg-white text-ink dark:bg-dark-bg dark:text-dark-ink">
      <PageHeader
        ...
        right={<><button onClick={() => navigate(`/stats?scope=${scope}&kind=${kind}`)} className="rounded-lg border border-line px-3 py-1 font-mono text-[10px] uppercase tracking-wider text-muted transition-colors hover:border-ink hover:text-ink dark:border-dark-line dark:text-dark-muted dark:hover:border-dark-muted dark:hover:text-dark-ink">stats →</button><MuteToggle /></>}
      >
        Game history
      </PageHeader>
      ...
      <main className="mx-auto max-w-6xl px-6 pb-10">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
          <ScopeToggle
            scope={scope}
            onChange={(s) => navigate(`/games?scope=${s}&kind=${kind}`)}
          />
          <div className="flex items-center gap-1 font-mono text-[10px]">
            {(["all", "human", "match"] as Kind[]).map((k) => (
              <button
                key={k}
                onClick={() => navigate(`/games?scope=${scope}&kind=${k}`)}
              ...
              >
                {k === "all" ? "all" : k === "human" ? "vs human" : "ai vs ai"}
              </button>
            ))}
          </div>
          <span className="font-mono text-[11px] text-muted dark:text-dark-muted">
            {filtered === null
              ? ""
              : filtersActive
                ? `${filtered.length} of ${games?.length ?? 0} on this page`
              ...
              >
                <div className="flex items-baseline justify-between font-mono text-[11px]">
                  <span className="text-ink dark:text-dark-ink">
                    {g.opponent_kind === "agent"
                      ? `${g.agent_id} vs ${g.opponent_agent_id ?? "?"}`
                      : `vs ${g.agent_id}`}
                  </span>
